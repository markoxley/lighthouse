...
func init() {
	...
	// Add one to WaitGroup for the processBuffer goroutine
	// This allows graceful shutdown to wait for all logs to be processed
	logger.wg.Add(1)
	...
	// This runs continuously until the buffer is closed during shutdown
...
// SetOutput sets a custom handler function for all log entries.
...
// SetDatabase configures database persistence for log entries.
...
func (l *Logger) write(level LogLevel, dt time.Time, message string, params map[string]string) {
	...
	if l.db != nil && l.insertString != "" {
		...
		l.db.Exec(l.insertString, dt.Unix(), level, level.String(), message, string(paramsJSON))
	}
}
